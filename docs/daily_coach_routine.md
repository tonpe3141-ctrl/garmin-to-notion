...
描画側は `days[]` から**質練習の本数と休養日数を自動で数え**、
`phase.weeklyTarget` / `phase.qualityTarget` / `phase.restTarget` と突き合わせて
達成度タイルに出す（足りなければオレンジになる）。質練習は `type` が
`"M"` / `"T"` / `"R"` の日だけを数える（`"E"` は数えない）。
**したがって `type` を正しく付けること。** Eジョグを `"M"` にすると本数が水増しされる。
...
2. データ構造は既存のものを厳密に踏襲する。キー名を変えない、増やさない、減らさない。
   ...
   - `phase` には `weeklyTarget` に加えて `qualityTarget`（例 `"週3本"`）と
     `restTarget`（例 `"週1日"`）も入れる。プロファイルのロードマップ表の値をそのまま写す。
     週プランの達成度タイルがこれと突き合わせる。
