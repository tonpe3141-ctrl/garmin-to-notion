...
2. データ構造は既存のものを厳密に踏襲する。キー名を変えない、増やさない、減らさない。
   ...
   **要点用のキー（2026-09-06 追加。必ず埋めること）:**
   ...
   - `today.points[]` — 要点。**4〜7個**。各要素は `{ tone, label, text }`。
     ...
     - `text` は **1〜2文**。数字を必ず入れる。3文以上書きたくなったら `verdictReason` へ回す
   - `axes[].summary` — その軸の結論を**1文**で。`text`（長文）は折りたたみの中に入る。
...
### STEP 6.5 — 文章の書き方（読ませるための規約）

**2026-09-06 の指摘:** 講評がだらだら長く、読むのが大変だった。
そこで描画側を「**数値とグラフ → 要点 → 全文（折りたたみ）**」の順に組み替えてある。
書く側もそれに合わせること。長く書くこと自体は禁止しないが、**長文は畳まれる前提で書く。**

**規約1 — 先頭の1文で意味が通るように書く。**
描画側は、要点キーが無い長文フィールドについては**先頭の1文だけを表示し、残りを折りたたむ**
（`。` までを機械的に切り出している）。したがって次のフィールドは、**1文目を結論にする**こと。
1文目が「良かったところから書きます。」のような前置きだと、畳まれた状態では何も伝わらない。

- `good[]` / `issues[]` の各項目（描画側が先頭文を太字にする）
- `tomorrow.why` / `tomorrow.next`
- `weekPlan.policy`
- `health.read`

**規約2 — 要点と全文で結論を変えない。**
`points` / `summary` は `verdictReason` / `axes[].text` の**要約**であって、別の主張ではない。
全文を開いた読者が「要点と違うことが書いてある」と感じたらそれは失敗。

**規約3 — 前置きを書かない。**
「〜から書きます」「まず〜を見ます」「〜を置いておきます」は畳まれた側に回すか、削る。
評価は結論から書く。

**規約4 — 過大評価しないという原則は変えない。**
短くすることと甘くすることは別。`headline` に「△ 要調整」を書かず良い面だけ並べるのは違反。
外した点は `tone: "bad"` で必ず立てる。
