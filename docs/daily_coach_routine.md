...
> 扱いは STEP 8 のとおり — 日々の DATA はコミットせず、描画レイヤー・この手順書・
...
1. **差し替えの土台は、STEP 3 で取得した「公開済み Artifact のソース」を使う。**
   ...
   **出来上がったものは `dashboard/index.html` に書き戻すこと**（リポジトリ側の
   古い中身は上書きしてよい。日々の DATA はコミットせず、STEP 8 で捨てる）。
...
### STEP 8 — 作業ツリーを戻す（日々の DATA はコミットしない）

**2026-09-06 に GitHub 連携が復旧し、push できるようになった。**
それでも**毎日の DATA はコミットしない。** 中身は Artifact（固定URL・版履歴あり）・
Notion・db に残っており、リポジトリに毎日 DATA の差分を積んでも履歴が汚れるだけである。
